...
// 查询工作督办详细
export function getWorkMain(id: string): Promise<Res<Work>> {
  return request('get', `/work/workMain/` + id);
}

// 新增工作督办
export function addWorkMain(data: Work): Promise<Res<void>> {
  return request('post', `/work/workMain`, { data: data });
}

// 修改工作督办
export function updateWorkMain(data: Work): Promise<Res<void>> {
  return request('put', `/work/workMain`, { data: data });
}

// 下达任务
export function distributeWork(id: number): Promise<Res<void>> {
  return request('put', `/work/workMain/distribute/${id}`);
}

// 撤回任务
export function withdrawWork(id: number): Promise<Res<void>> {
  return request('put', `/work/workMain/withdraw/${id}`);
}

// 督查任务
export function supervisionWork(data: WorkFeedback[]): Promise<Res<void>> {
  return request('put', `/work/workMain/supervision`, { data: data });
}

// 退回任务
export function recallWork(data: WorkFeedback[]): Promise<Res<void>> {
  return request('put', `/work/workMain/recall`, { data: data });
}

// 催办任务
export function urgingWork(data: WorkFeedback[]): Promise<Res<void>> {
  return request('put', `/work/workMain/urging`, { data: data });
}

// 删除工作督办
export function delWorkMain(id: number | number[]): Promise<Res<void>> {
