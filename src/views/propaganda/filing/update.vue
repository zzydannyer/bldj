...
import StandardPicker from '@/views/propaganda/filing/standardPicker.tsx'
...
const getPropagandaLsit = async () => {
  try {
    ...
    let scoreDetail = (isDepartment.value
      ? data.scoreDetailId
      : data.groupScoreDetailId)?.split(',')
  ...
  <section class="container">
    <van-form input-align="left" @submit="onSubmit">
      <van-cell-group inset>
        <!-- 赋分依据 -->
        <standard-picker v-model:scoreDetailId="form.scoreDetailId" @load="handleTreeOptionsLoad"
       ...
       -->
        ...
        <v-date-picker required v-model="form.reportTime" :min-date="new Date(startTime)" :max-date="new Date()"
          label="报道日期" :rules="[{ required: true, message: '请选择报道日期' }]" placeholder="请选择报道日期" />

        <!-- 外部链接 -->
        <van-field v-model="form.outLink" :rules="[{ validator, message: '请填写正确链接地址' }]" label="外部链接"
          placeholder="请输入外部链接" />
        <van-field v-model="mediatype[form.fileType]" is-link readonly name="picker" label="资源类型" placeholder="点击资源类型"
