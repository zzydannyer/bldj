...
import {
  listPublicResourceFile,
  delPublicResourceFile,
  updataIsHomeShow,
} from '@/api/media/publicResource'
...
<template>
  <main class="list-container">
    <v-search
        :showPopIcon="false"
        placeholder="请输入公共资源名称"
        v-model:searchVal="queryParams.resTitle"
        @handle-search="handleSearch"
    />

    <v-inset-list
        :shows="['search']"
        :list-fn="listPublicResourceFile"
          ...
          <section class="flex-1">
            ...
            <div class="mt-2">
              <v-tag
                  ...
                  plain
                  :dictData="public_resourses_type"
                  :value="row.fileType"
              />
              <van-switch
                  size="14px"
    ...
    <van-floating-bubble
        axis="xy"
        magnetic="x"
        v-if="hasAuth('publicresource:publicResourceFile:add')"
    >
      <van-button
          type="primary"
          size="large"
          round
          to="/public/resource/create"
      >
        <van-swipe
            vertical
            class="button-swipe"
