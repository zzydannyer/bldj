<script setup lang="ts">
  ...
  const isCompany = computed(() => roles?.value?.includes('company-jbr'));
  // 是否为后台管理
  ...
  const handleGroup = async (id: number) => {
    router.push('/propaganda/approval/create/' + id + '/' + '1003');
  };
</script>
...
<template>
  <main class="list-container">
    ...
    <v-inset-list :shows="['search', 'tabs']" :list-fn="listAuditPropaganda" :query-params="queryParams" ref="listRef">
        ...
        <v-card>
          ...
          <section class="absolute right-2 bottom-2">
            ...
            <van-button
              ...
              type="primary"
              @click="router.push('/propaganda/approval/audit/' + row.id + '/' + 1)"
            />
            <van-button
              class="px-4"
              round
