...
import { Button, Image, Dialog, Field, NumberKeyboard, Loading, CheckboxGroup, Checkbox, Picker, Popup, Overlay, PickerOption } from 'vant';
    ...
    return () => (
      <>
          ...
          {props.mediaList.map((media, index) => (
            <>
              <div class="media-container">
                ...
                <Checkbox class="review-check" name={media.id}>
                  {{
                    icon: ({ checked }: { checked: boolean }) => (
                      <>
                        <div class={checked ? 'review-checked' : 'review-unchecked'} />
                        <Icon class="absolute left-0 top-0" icon="radix-icons:check" color="white" />
                      </>
                    )
                  }}
                </Checkbox>
                ...
                <div class="preview-cover van-ellipsis" onClick={withModifiers(() => handleNameClick(index), ['stop'])}>
                ...
                />
                <Button v-show={props.type === 'video'} class="w-32" plain hairline size="mini" type="primary" onClick={() => handleVideoClip(media)}>
                  裁剪
                </Button>
              </div>
            </>
        ...
        <Popup v-model:show={showPicker.value} round position="bottom">
          <Picker columns={scorevalue} onCancel={onCancel} onConfirm={onChange} columns-field-names={{ text: 'label', value: 'value' }} />
        </Popup>

        {/* 预览 */}
        <VPreview v-model={showPreview.value} type={props.type} list={props.mediaList} mediaIndex={mediaIndex.value} />

        {/* 改名 */}
        <Dialog v-model:show={showChangeName.value} title="修改素材名称" show-cancel-button teleport="body" onConfirm={handleConfirm}>
          <Field class="py-8" v-model={mediaName.value} input-align="center" placeholder="请输入文件名" />
        </Dialog>
