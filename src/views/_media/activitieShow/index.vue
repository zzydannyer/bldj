...
<template>
  <main class="list-container">
    <v-search placeholder="请输入活动名称" v-model:searchVal="queryParams.activitiesName" @handle-search="handleSearch">
      <template #popMenu>
    ...
    <v-inset-list :shows="['search']" :list-fn="listPublicMediaActivitiesScore" :query-params="queryParams" ref="listRef">
      <template #default="{ row }">
        <v-card body-class="flex flex-between gap-2" @click="handleDetail(row.id, row.activitiesId)">
