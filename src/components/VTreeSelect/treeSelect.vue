...
const searchFn = debounce((searchQuery, callback) => {
    searchUser(0, searchQuery)
        .then(({ data }) => {
    ...
    if (action === "LOAD_CHILDREN_OPTIONS") {
        if (parentNode.type === 3 && importType === 'user') {
            listUserByPosition(parentNode.id)
                .then(({ data }) => {
                    if (data && data.length) {
                        parentNode.children = data.map((item: SysUser) => ({
                            id: item.userId,
                            code: item.orgCode,
                            org: item.orgName,
                            label: item.nickName,
            ...
            listOrgChildren(parentNode.id)
                .then(({ data }) => {
                    if (data && data.length) {
                        ...
                        parentNode.children = result.map((item: SysOrg) => ({
                            id: item.id,
                            code: item.code,
                            label: item.name,
                            parentId: parentNode.id,
        ...
        listOrgChildren(0)
            .then(({ data }) => {
                if (data && data.length) {
                    options.value = data.map((item: SysOrg) => ({
                        id: item.id,
                        code: item.code,
                        label: item.name,
                        parentId: 0,
