<template>
  ...
  <van-overlay :show="modelValue" z-index="2">
    ...
    <!-- 视频关闭按钮 -->
    <van-icon v-if="type === 'video'" class="p-4 float-right" name="cross" color="white" size="20px"
    ...
    <!-- 轮播 -->
    <swiper-container v-if="type === 'image'" ref="target" class="preview-swiper" :slides-per-view="1" :slides-per-group="1"
      :centered-slides="false" :pagination="{
        el: '#preview-pagination',
        hideOnClick: false,
        type: 'progressbar',
      }" @init="init" @slidechange="onSlideChange">
